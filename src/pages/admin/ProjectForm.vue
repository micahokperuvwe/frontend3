<template>
  <div class="min-h-screen bg-transparent  py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Back Link -->
      <router-link to="/admin/projects" class="inline-flex items-center gap-2 text-sm text-[var(--color-text-main)]  hover:underline mb-6 font-medium">
        ← Back to Projects List
      </router-link>

      <!-- Header -->
      <h1 class="text-3xl font-bold text-[var(--color-text-main)]  mb-8">
        {{ isEditMode ? 'Edit Project' : 'Create New Project' }}
      </h1>

      <form @submit.prevent="submitForm" class="bg-[var(--color-bg-main)]  rounded-2xl shadow-md p-8 space-y-6">
        <!-- Loading state -->
        ...
        <div v-else class="space-y-6">
          <!-- Title -->
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-main)]  mb-2">Project Title *</label>
            <input
              v-model="form.title"
              type="text"
              required
              ...
              class="w-full px-4 py-2 border border-[var(--color-border)]/20 dark:border-gray-600 rounded-2xl bg-[var(--color-bg-main)] dark:bg-gray-700 text-[var(--color-text-main)]  focus:ring-2 focus:ring-blue-500 outline-none"
            />
          ...
          <!-- Slug -->
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-main)]  mb-2">URL Slug *</label>
            <input
              v-model="form.slug"
              type="text"
              required
          ...
          <!-- Short Description -->
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-main)]  mb-2">Short Description *</label>
            <input
          ...
          <!-- Full Description -->
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-main)]  mb-2">Full Description *</label>
            <textarea
              v-model="form.full_description"
              required
              rows="5"
              placeholder="Detailed overview of project goals, features, and technical results..."
              class="w-full px-4 py-2 border border-[var(--color-border)]/20 dark:border-gray-600 rounded-2xl bg-[var(--color-bg-main)] dark:bg-gray-700 text-[var(--color-text-main)]  focus:ring-2 focus:ring-blue-500 outline-none resize-none"
            ></textarea>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-[var(--color-text-main)]  mb-2">Category *</label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-[var(--color-text-main)]  mb-2">Status *</label>
              <select
                v-model="form.status"
                class="w-full px-4 py-2 border border-[var(--color-border)]/20 dark:border-gray-600 rounded-2xl bg-[var(--color-bg-main)] dark:bg-gray-700 text-[var(--color-text-main)]  focus:ring-2 focus:ring-blue-500 outline-none"
              >
          ...
          <!-- Technologies -->
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-main)]  mb-2">Technologies (comma-separated) *</label>
            <input
          ...
          <!-- Live Demo URL -->
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-main)]  mb-2">Live Demo URL (Optional)</label>
            <input
          ...
          <div class="border-t border-[var(--color-border)]/20  pt-6">
            <label class="block text-sm font-semibold text-[var(--color-text-main)]  mb-2">Main Cover Media (Image or Video)</label>
            <div class="flex items-center gap-4">
              <div v-if="form.thumbnail_image" class="w-28 h-28 rounded-2xl overflow-hidden border border-[var(--color-border)]/20 dark:border-gray-600 relative shrink-0 bg-gray-900 flex items-center justify-center">
              ...
              <div class="flex-1 space-y-2">
                <input
                  ...
                  class="block w-full text-sm text-[var(--color-text-muted)]  file:mr-4 file:py-2 file:px-4 file:rounded-2xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 dark:file:bg-blue-900/40 dark:file:text-blue-300 hover:file:bg-blue-100"
                />
                <div v-if="uploadingThumbnail" class="text-xs text-[var(--color-text-main)] ">Uploading cover media...</div>
                <input
                  v-model="form.thumbnail_image"
                  type="url"
                  placeholder="Or paste direct image/video URL (https://...)"
                  class="w-full px-3 py-1.5 text-xs border border-[var(--color-border)]/20 dark:border-gray-600 rounded-xl bg-[var(--color-bg-main)] dark:bg-gray-700 text-[var(--color-text-main)] "
                />
              </div>
            </div>
          </div>
          ...
          <div class="border-t border-[var(--color-border)]/20  pt-6">
            <div class="flex items-center justify-between mb-3">
              <div>
                <label class="block text-sm font-semibold text-[var(--color-text-main)] ">Project Gallery Media - Images & Videos (Max 10)</label>
                <p class="text-xs text-[var(--color-text-muted)] ">Upload up to 10 photos or video clips for this project gallery.</p>
              </div>
            ...
            <div v-if="galleryImages.length < 10" class="mb-4">
              <input
                ...
                class="block w-full text-sm text-[var(--color-text-muted)]  file:mr-4 file:py-2 file:px-4 file:rounded-2xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 dark:file:bg-blue-900/40 dark:file:text-blue-300 hover:file:bg-blue-100"
              />
              <div v-if="uploadingGallery" class="text-xs text-[var(--color-text-main)]  mt-1 font-semibold">Uploading media file(s)... Please wait.</div>
            </div>
            ...
            <div v-if="galleryImages.length > 0" class="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div
                ...
                :key="idx"
                class="relative rounded-2xl overflow-hidden border border-[var(--color-border)]/20  group h-32 bg-gray-900 flex items-center justify-center"
              >
          ...
          <!-- Checkboxes -->
          <div class="flex gap-6 border-t border-[var(--color-border)]/20  pt-6">
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="form.published" type="checkbox" class="w-4 h-4 text-[var(--color-text-main)] rounded" />
              <span class="text-sm font-medium text-[var(--color-text-main)] ">Publish Immediately</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="form.featured" type="checkbox" class="w-4 h-4 text-[var(--color-text-main)] rounded" />
              <span class="text-sm font-medium text-[var(--color-text-main)] ">Feature on Homepage</span>
            </label>
          </div>

          <!-- Status Message -->
          <div v-if="statusMessage" :class="`p-4 rounded-2xl text-sm font-medium ${statusMessage.type === 'success' ? 'bg-green-100 dark:bg-green-900/60 text-green-800 dark:text-green-200 border border-green-300' : 'bg-red-100 dark:bg-red-900/60 text-red-800 dark:text-red-200 border border-red-300'}`">
            {{ statusMessage.text }}
          </div>
          ...
          <button
            ...
            class="w-full px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold rounded-2xl transition-colors shadow"
          >
